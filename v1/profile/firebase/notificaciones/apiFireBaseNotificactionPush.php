...
//$cliente= $_POST['cliente'];
    ...
    try
    {
        ...
        $actionDestination = isset($_POST['action_destination'])?$_POST['action_destination']:'';
            ...
            //$server_apyKey = "AAAAgzEUuzA:APA91bEYItPik6a7SfEJM3QG3L1fLASyOk7p69rGUFHZX2jXMus03CeM3TPaQkCPBMEmL-8np8wZdKQPJY4erbWYHSkN5gpt06Jfgs98KdKLZkrFa9vHhI0BmTY6bUij9HQWAXOUmYhG";
            $server_apyKey = "AAAATaAAt-Q:APA91bGfgFju-b1J6xkSXpyRaSSdV12cqb6dn42V2erSS84uDYMQR3X-7LXzDIIHIwEqg-f_iA7RFYCQChlSvqcr4V5qCIi56HbAkbB8-U7N37d966y8ZRlXz43Y_ai9dRPqlh1zR2J2";
            $topic = $_POST['topic'];
            ...
            $requestData = $notification->getNotificatin();
                    ...
                    //var_dump($json_result->{'results'}[0]->{"error"}) ;'Msg'=>$json_result
                    if($json_result->{'success'} == 1) {
                        $json=array('Respuesta'=>'Ok','Msg'=>'Mensaje enviado','token'=>$token);
                        echo json_encode($json);
                    }
                     else{
                        $json=array('Respuesta'=>'Error','Msg'=>$json_result->{'results'}[0]->{"error"},'token'=>$token);
                        echo json_encode($json);
                     }
