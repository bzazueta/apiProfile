    ...
    try
    {
        ...
        $actionDestination = isset($_POST['action_destination'])?$_POST['action_destination']:'';
            ...
            $requestData = $notification->getNotificatin();
                    ...
                    //var_dump($json_result->{'results'}[0]->{"error"}) ;
                    if($json_result->{'success'} == 1) {
                        $json=array('Respuesta'=>'Ok','Msg'=>$json_result,'message'=>$token);
                        echo json_encode($json);
                    }
                     else{
                        $json=array('Respuesta'=>'Error','Msg'=>$json_result->{'results'}[0]->{"error"},'message'=>$token);
                        echo json_encode($json);
                     }
